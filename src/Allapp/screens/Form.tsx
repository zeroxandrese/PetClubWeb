...
import { BusinessRegisterData } from '../interface/interface';
import { useNavigate } from 'react-router-dom';

import { AuthContext } from '../context/AuthContext';
import arrow from '../assets/arrowBackPNG.png';
...
export const Form: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    } else if (!weekendStartTime || !weekStartTime) {
      isValid = false;
      errorMessage = 'Debes seleccionar las horas.';
    }
    ...
    const formData: BusinessRegisterData = {
      typeUser: selectedBusinessType.join(', ') + ' ' + selectedBusinessType2.join(', '),
      name,
  ...
  return (
      ...
      <button
        onClick={() => navigate('/')} style={{ background: '000000', alignItems: 'center', width: 25, height: 25 }}>
        <img src={arrow} />
      </button>
